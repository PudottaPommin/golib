...
import (
	...
	"github.com/pudottapommin/golib/pkg/hasher"
)
...
func TestMainHasherHash(t *testing.T) {
	...
	for _, p := range pairs {
		t.Run("Hash["+p+"]", func(t *testing.T) {
			...
			if err != nil && result != hasher.PasswordVerificationSuccess {
				t.Errorf("Argon2id:Hash verification generated for %v with %v", p, err)
			}

			if result != hasher.PasswordVerificationSuccess {
				t.Errorf("Argon2id:Hash failed to verify result: %d != %d", hasher.PasswordVerificationSuccess, result)
			}
...
func TestHasherVerify(t *testing.T) {
	...
	for _, p := range pairs {
		t.Run("Verify["+p.password+"]", func(t *testing.T) {
			...
			if err != nil && p.result != hasher.PasswordVerificationFailed {
...
func TestPbkdf2HasherHash(t *testing.T) {
	...
	for _, p := range pairs {
		t.Run("Hash["+p+"]", func(t *testing.T) {
			...
			if result != hasher.PasswordVerificationSuccess {
				t.Errorf("Pbkdf2:Hash failed to verify result: %d != %d, err: %v", hasher.PasswordVerificationSuccess, result, err)
			}
		})
	}
}
