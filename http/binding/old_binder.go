...
type ValueBinder struct {
	...
	// ValuesFunc is used to get all values for a parameter from the request. i.e. `/api/search?ids=1&ids=2`
...
// BindError returns the first seen bind error and resets/empties binder errors for further calls
...
func (b *ValueBinder) ShouldUUIDs(key string, dest *[]uuid.UUID) *ValueBinder {
	return b.uuids(key, dest, false)
}

func (b *ValueBinder) UUIDs(key string, dest *[]uuid.UUID) *ValueBinder {
	return b.uuids(key, dest, true)
}
...
func (b *ValueBinder) uuids(key string, dest *[]uuid.UUID, valueMustExist bool) *ValueBinder {
	if b.failFast && b.errors != nil {
		return b
	}

	values := b.ValuesFunc(key)
	if len(values) == 0 {
		if valueMustExist {
			b.setError(b.ErrorFunc(key, []string{}, ErrRequired.Error(), nil))
		}
		return b
	}
	...
	for _, v := range values {
		...
		if err != nil {
			b.setError(b.ErrorFunc(key, []string{v}, ErrInvalidUUID.Error(), nil))
			return b
		}
		res = append(res, id)
	}
...
func (b *ValueBinder) ShouldTimes(key string, dest *[]time.Time, layout string) *ValueBinder {
	return b.times(key, dest, layout, false)
}

func (b *ValueBinder) Times(key string, dest *[]time.Time, layout string) *ValueBinder {
	return b.times(key, dest, layout, true)
}
...
func (b *ValueBinder) ShouldDates(key string, dest *[]time.Time) *ValueBinder {
	return b.times(key, dest, time.DateOnly, false)
}

func (b *ValueBinder) Dates(key string, dest *[]time.Time) *ValueBinder {
	return b.times(key, dest, time.DateOnly, true)
}
...
func (b *ValueBinder) times(key string, dest *[]time.Time, layout string, valueMustExist bool) *ValueBinder {
	if b.failFast && b.errors != nil {
		return b
	}
	...
	res := make([]time.Time, 0, len(values))
	for _, v := range values {
		if layout == time.TimeOnly && strings.Count(v, ":") == 1 {
			v += ":00"
		}
		t, err := time.Parse(layout, v)
		if err != nil {
			b.setError(b.ErrorFunc(key, []string{v}, ErrInvalidTime.Error(), err))
			return b
		}
		res = append(res, t)
	}
...
// ShouldStrings binds parameter to string slice
func (b *ValueBinder) ShouldStrings(sourceParam string, dest *[]string) *ValueBinder {
	if b.failFast && b.errors != nil {
		return b
	}
...
// Strings requires parameter values to exist to bind to string slice. Returns error when no values exist
...
func (b *ValueBinder) ShouldBools(sourceParam string, dest *[]bool) *ValueBinder {
	return b.boolValues(sourceParam, dest, false)
}

func (b *ValueBinder) Bools(sourceParam string, dest *[]bool) *ValueBinder {
	return b.boolValues(sourceParam, dest, true)
}
...
func (b *ValueBinder) boolValues(sourceParam string, dest *[]bool, valueMustExist bool) *ValueBinder {
	if b.failFast && b.errors != nil {
		return b
	}
	...
	for _, v := range values {
		...
		if err != nil {
			b.setError(b.ErrorFunc(sourceParam, []string{v}, ErrInvalidBool.Error(), err))
			return b
		}
		res = append(res, n)
	}
...
func (b *ValueBinder) ShouldCustoms(sourceParam string, factory func() IBindable, dest *[]IBindable) *ValueBinder {
	return b.customs(sourceParam, factory, dest, false)
}
...
func (b *ValueBinder) Customs(sourceParam string, factory func() IBindable, dest *[]IBindable) *ValueBinder {
	return b.customs(sourceParam, factory, dest, true)
}

func (b *ValueBinder) customs(sourceParam string, factory func() IBindable, dest *[]IBindable, valueMustExist bool) *ValueBinder {
	if b.failFast && b.errors != nil {
		return b
	}
	...
	res := make([]IBindable, 0, len(values))
	for _, v := range values {
		d := factory()
		if err := d.Parse(v); err != nil {
			b.setError(b.ErrorFunc(sourceParam, []string{v}, ErrInvalidBindable.Error(), err))
			return b
		}
		res = append(res, d)
	}
...
func (b *ValueBinder) CustomsFunc(sourceParam string, fn func(string) error) *ValueBinder {
	if b.failFast && b.errors != nil {
		return b
	}
