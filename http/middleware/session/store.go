...
type Store interface {
	...
	Save(w http.ResponseWriter, id []byte) error
}
...
func (cs *cookieStore) Get(r *http.Request) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("[Session] failed to get cookie: %w", err)
	}
	if err = ck.Valid(); err != nil {
		return nil, fmt.Errorf("[Session] cookie is invalid: %w", err)
	}
...
func (cs *cookieStore) Save(w http.ResponseWriter, token []byte) error {
	...
	if err != nil {
		return fmt.Errorf("[Session] failed to secure token: %w", err)
	}
	ck := &http.Cookie{
		Name:     cs.name,
